...
readonly HAPROXY_SERVER_NAME="serena-mcp"
...
handle_first_run() {
    ...
    # Debian slim base — use groupadd/useradd (not alpine addgroup/adduser)
    if ! id -u serena >/dev/null 2>&1; then
        groupadd -g "$PGID" serena 2>/dev/null || true
        useradd -u "$PUID" -g "$PGID" -s /bin/bash -d /home/serena -m serena 2>/dev/null || true
    fi
    ...
    if id -u serena >/dev/null 2>&1; then
        if [ "$(id -u serena)" -ne "$PUID" ]; then
            if usermod -o -u "$PUID" serena 2>/dev/null; then
                uid_gid_changed=1
            else
                PUID="$(id -u serena)"
            fi
        ...
        if [ "$(id -g serena)" -ne "$PGID" ]; then
            if groupmod -o -g "$PGID" serena 2>/dev/null; then
                uid_gid_changed=1
            else
                PGID="$(id -g serena)"
            fi
    ...
    # Ensure data and config dirs are writable
...
"
    ...
    # saturate the upstream stdio bridge or trigger unbounded child spawns
    # in stateless modes. Empty = no cap.
...
build_serena_cmd() {
    # Assemble serena CLI args from env
    SERENA_ARGS=(start-mcp-server
          --context "${SERENA_CONTEXT:-desktop-app}"
          --transport "${SERENA_TRANSPORT:-stdio}")

    if [[ -n "${SERENA_PROJECT:-}" && -d "${SERENA_PROJECT}" ]]; then
        SERENA_ARGS+=(--project "${SERENA_PROJECT}")
    else
        SERENA_ARGS+=(--project-from-cwd)
    fi

    # Non-stdio transports need host/port
    if [[ "${SERENA_TRANSPORT}" != "stdio" ]]; then
        SERENA_ARGS+=(--host 0.0.0.0 --port "${SERENA_PORT:-9121}")
    fi

    # Optional: modes
    if [[ -n "${SERENA_MODES:-}" ]]; then
        for m in ${SERENA_MODES//,/ }; do
            SERENA_ARGS+=(--mode "$m")
        done
    fi

    # Optional: log level
    [[ -n "${SERENA_LOG_LEVEL:-}" ]] && SERENA_ARGS+=(--log-level "${SERENA_LOG_LEVEL}")
}

start_mcp_server() {
    # Serena env defaults
    SERENA_PROJECT="${SERENA_PROJECT:-/data}"
    SERENA_CONTEXT="${SERENA_CONTEXT:-desktop-app}"
    SERENA_TRANSPORT="${SERENA_TRANSPORT:-stdio}"
    SERENA_PORT="${SERENA_PORT:-9121}"
    SERENA_MODES="${SERENA_MODES:-}"
    SERENA_LOG_LEVEL="${SERENA_LOG_LEVEL:-INFO}"

    # mcp-proxy session model: stateful by default — one stdio child per
    # Mcp-Session-Id, reused across requests. Set MCP_PROXY_STATELESS=true
    # only when full per-request isolation is required (memory-hostile).
    MCP_PROXY_STATELESS="${MCP_PROXY_STATELESS:-false}"
    # Cap virtual memory of each stdio child (0 = disabled).
    SERENA_MAX_MEM_MB="${SERENA_MAX_MEM_MB:-0}"

    build_serena_cmd

    # mcp-proxy receives the stdio command as positional args after `--`,
    # so prlimit can prefix the argv list directly.
    local serena_argv=(serena "${SERENA_ARGS[@]}")
    if [[ "${SERENA_MAX_MEM_MB}" =~ ^[1-9][0-9]*$ ]] && command -v prlimit >/dev/null 2>&1; then
        local mem_bytes=$((SERENA_MAX_MEM_MB * 1024 * 1024))
        serena_argv=(prlimit "--as=${mem_bytes}" -- "${serena_argv[@]}")
        echo "Serena child memory cap: ${SERENA_MAX_MEM_MB} MiB (prlimit --as)"
    fi

    # Change to project dir
    cd "${SERENA_PROJECT:-/data}" 2>/dev/null || cd /
    ...
    case "${PROTOCOL^^}" in
        ...
        WS|WEBSOCKET)
            ...
            ;;
        STDIO)
            # Pure stdio mode — bypass HAProxy and mcp-proxy entirely
            echo "Launching Serena MCP in pure stdio mode"
            if [ "$(id -u)" -eq 0 ] && id -u serena >/dev/null 2>&1; then
                exec gosu serena serena "${SERENA_ARGS[@]}" "$@"
            else
                exec serena "${SERENA_ARGS[@]}" "$@"
            fi
            ;;
        *)
    ...
    echo "Launching Serena MCP with protocol: ${PROTOCOL_DISPLAY}"

    if [ "$(id -u)" -eq 0 ] && id -u serena >/dev/null 2>&1; then
        gosu serena "${CMD_ARGS[@]}" &
    else
        "${CMD_ARGS[@]}" &
    fi
...
main() {
    ...
    # STDIO mode exits early — no HAProxy needed
    if [[ "${PROTOCOL^^}" == "STDIO" ]]; then
        start_mcp_server "$@"
        # start_mcp_server exec's in STDIO mode, so we never reach here
        exit 0
    fi
